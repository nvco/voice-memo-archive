...
`import_mode`/`import_since`/`setup_completed_at` were added in Phase 7
(`tasks/035-...md`) for the setup flow. They use `.get()` with defaults in
...
`schedule_mode`/`scan_interval_seconds` existed here too, for the
`launchd` background-automation feature removed in
`tasks/050-remove-background-automation.md`. `from_dict` silently ignores
either key if still present in an old `config.json` (unknown keys are
simply not read), so no migration was needed to remove them.
"""
