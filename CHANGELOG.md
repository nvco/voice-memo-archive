...
- Retired the project: development has stopped and `README.md` now says
  it is no longer maintained. Recording straight into the destination app
  makes the Voice Memos export step unnecessary. The tool still works for
  exporting existing transcripts, but no further changes are planned.
